...
with rs_tab :
    ...
    if "spend grouped by account" in s_type.lower () :
        base_prompt = """ write a python script to find current aws spend per day for the last 10 days, pass groupby the Type DIMENSION and Key  LINKED_ACCOUNT into get_cost_and_usage() api call, give the results in a pandas dataframe . comment the code and remove all empty lines from the function. the start time argument to get_cost_and_usage format should be yyyy-MM-dd with start date 10 days back and end date of yesterday.  do not use python functions """

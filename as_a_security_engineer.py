...
with rs_tab :
    ...
    if "spend grouped by account" in s_type.lower () :
        base_prompt = """ write a python function with to find current aws spend per day for the last 10 days, pass groupby dimension of LINKED_ACCOUNT into get_cost_and_usage() api call, give the results in a pandas dataframe . comment the code and remove all empty lines from the code. the start time argument to get_cost_and_usage format should be yyyy-MM-dd.  use UnblendedCost for Metrics. always use spaces as indentation in python """
